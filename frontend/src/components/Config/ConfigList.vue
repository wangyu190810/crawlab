  ...
  <div class="config-list">
    ...
    >
      <el-table class="table-header" :data="[{}]" :show-header="false">
        <el-table-column v-for="(f, index) in fields" :key="f.name + '-' + index" min-width="100px">
          <template>
      ...
      <el-table :data="previewCrawlData" :show-header="false" border>
        <el-table-column
          ...
          min-width="100px"
        >
          <template slot-scope="scope">{{getDisplayStr(scope.row[f.name])}}</template>
        </el-table-column>
      </el-table>
    </el-dialog>
    <!--./preview results-->
    ...
    <el-tabs :active-name="activeTab" @tab-click="onTabClick">
      ...
      <el-tab-pane name="stages" :label="$t('Stages')">
        <!--config detail-->
        <el-row>
          <el-form label-width="150px" ref="form" :model="spiderForm.config"></el-form>
        </el-row>
        <!--./config detail-->
            ...
            <ul class="list">
              ...
              <li class="item">
                ...
                <el-select
                  ...
                  disabled
                >
                  <el-option v-for="n in ['scrapy','data']" :key="n" :value="n" :label="n" />
                </el-select>
              </li>
              <li class="item">
                <label>{{$t('Proxy')}}:</label>
                <el-select
                  v-model="spiderForm.config.proxy"
                  :placeholder="$t('Proxy')"
                >
                  <el-option v-for="n in proxy" :key="n" :value="n" :label="n"></el-option>
                </el-select>
              </li>
              <li class="item">
                <label>{{$t('Goose')}}:</label>
              ...
              <el-select
                  v-model="spiderForm.config.goose"
                  :placeholder="$t('Goose')"
                >
                  <el-option v-for="(n,index_g) in goose" 
                  :key="index_g" :value="n.value" :label="n.label">
                    </el-option>
                </el-select>
              ...
              <li class="item">
                ...
                <div class="selector-type">
                  <span class="selector-type-item" @click="onClickSelectorType('css')">
                    <el-tag :class="isCss ? 'active' : 'inactive'" type="success">CSS</el-tag>
                  </span>
                  <span class="selector-type-item" @click="onClickSelectorType('xpath')">
                    <el-tag :class="isXpath ? 'active' : 'inactive'" type="primary">XPath</el-tag>
                  </span>
                </div>
              </li>
            </ul>
      ...
      <el-tab-pane name="settings" :label="$t('Settings')">
        <div class="actions" style="text-align: right;margin-bottom: 10px">
          <el-button type="success" size="small" @click="onSave">{{$t('Save')}}</el-button>
        </div>
        <setting-fields-table-view type="list" />
      </el-tab-pane>
      <!--./Setting-->
...
export default {
    ...
    return {
      ...
      tourSteps: [
        ...
        },
        {
          target: "#start-stage",
          content: this.$t(
            "Here is the starting stage of the spider.<br><br>A <strong>Stage</strong> is basically a callback in the Scrapy spider."
          )
        },
        {
          target: "#btn-run",
          content: this.$t(
            "You can run a spider task.<br><br>Spider will be automatically saved when clicking on this button."
          )
        },
        {
        ...
        },
        {
          ...
          content: this.$t(
            'You can decide whether this is a list page.<br><br>Click on the CSS/XPath tag to enter the selector expression for list items.<br>For example, "<code>ul > li</code>"'
          ),
          params: {
            placement: "top"
          }
        },
        {
          ...
          content: this.$t(
            'You can decide whether this is a list page with pagination.<br><br>Click on the CSS/XPath tag to enter the selector expression for the pagination.<br>For example, "<code>a.next</code>"'
          ),
          params: {
            placement: "top"
          }
        ...
        // process
        {
          target: "#tab-process",
          content: this.$t(
            "You can view the<br> visualization of the stage<br> workflow."
          )
        },
        // settings
        {
          target: "#tab-settings",
          content: this.$t(
            "You can add the settings here, which will be loaded in the Scrapy's <code>settings.py</code> file.<br><br>JSON and Array data are supported."
          )
        },
        // Spiderfile
        {
          ...
          content: this.$t(
            'You can edit the <code>Spiderfile</code> here.<br><br>For more information, please refer to the <a href="https://docs.crawlab.cn/Usage/Spider/ConfigurableSpider.html" target="_blank" style="color: #409EFF">Documentation (Chinese)</a>.'
          )
        }
      ],
      tourCallbacks: {
  ...
  computed: {
    ...
    stageNodes() {
      ...
      // 加入剩余阶段
      i = 0;
      const restStages = this.spiderForm.config.stages.filter(
        stage => !allStageNames.has(stage.name)
      );
            ...
            this.$store
              ...
              .then(response => {
                ...
                if (response.data.pagination_selector) {
                  this.spiderForm.pagination_selector =
                    response.data.pagination_selector;
                }
              })
              .finally(() => {
        ...
        tooltip: {
          ...
          formatter: params => {
            ...
            if (params.data.list_css || params.data.list_xpath) {
              str += `<li><span style="display: inline-block;min-width: 50px;font-weight: bolder;text-align: right;margin-right: 3px">${this.$t(
                "List"
              )}: </span>${params.data.list_css ||
                params.data.list_xpath}</li>`;
            }
            if (params.data.page_css || params.data.page_xpath) {
              str += `<li><span style="display: inline-block;min-width: 50px;font-weight: bolder;text-align: right;margin-right: 3px">${this.$t(
                "Pagination"
              )}: </span>${params.data.page_css ||
                params.data.page_xpath}</li>`;
            }
            str += `</ul><br>`;
